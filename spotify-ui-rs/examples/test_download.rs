...
fn validate_duration(file: &Path, spotify_ms: Option<i64>) -> Result<i64, String> {
    ...
    if let Some(expected) = spotify_ms {
        ...
        if diff > DURATION_REJECT_THRESHOLD_MS {
            return Err(format!(
                "duration mismatch: spotify={}ms file={}ms diff={}ms",
                expected, file_ms, diff
            ));
        }
    }
    Ok(file_ms)
...
fn main() {
    let test_cases = vec![
        TestCase {
            ...
            spotify_duration_ms: 467_000,
        },
        TestCase {
            artist: "Radiohead",
            track: "Creep",
            spotify_duration_ms: 236_000,
        },
        TestCase {
            artist: "久石譲",
            track: "Summer",
            spotify_duration_ms: 245_000,
        },
        TestCase {
            artist: "Adele",
            track: "Hello",
            spotify_duration_ms: 295_000,
        },
        TestCase {
            artist: "Jay Chou",
            track: "Mojito",
            spotify_duration_ms: 194_000,
        },
    ];
    ...
    println!(
        "Mode: {}",
        if download_mode {
            "search + download + validate"
        } else {
            "search + score only (use --download to also download)"
        }
    );
    println!();
    ...
    for tc in &test_cases {
        ...
        println!("--- {} ---", query);
        println!(
            "  Spotify duration: {}ms ({:.1}s)",
            tc.spotify_duration_ms,
            tc.spotify_duration_ms as f64 / 1000.0
        );

        let req = DownloadRequest {
            track_name: tc.track.to_string(),
            _artist_name: tc.artist.to_string(),
        ...
        if download_mode {
            ...
            println!(
                "  Downloading best candidate (score={:.1}, id={})...",
                best_score, best.id
            );
            if download_candidate(&best.id, &output_path) {
                match validate_duration(&output_path, Some(tc.spotify_duration_ms)) {
                    Ok(file_ms) => {
                        ...
                        println!(
                            "  [OK] Downloaded & validated: file={}ms diff={}ms",
                            file_ms, diff
                        );
                    }
                    Err(reason) => {
